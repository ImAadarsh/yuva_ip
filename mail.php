...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $required_fields = ['school_name', 'city', 'principal_name', 'principal_email', 
                       'coordinator_name', 'coordinator_designation', 'coordinator_phone', 'coordinator_email'];
    ...
    if (!filter_var($principal_email, FILTER_VALIDATE_EMAIL) || !filter_var($coordinator_email, FILTER_VALIDATE_EMAIL)) {
        echo json_encode(['message' => 'Please enter valid email addresses', 'success' => false]);
        exit;
    }
    
    // Check if school already exists using prepared statement
    $school_check_query = "SELECT * FROM yuva WHERE school_name = ? AND city = ?";
    ...
    if ($result->num_rows > 0) {
        echo json_encode(['message' => 'This school is already registered for the YUVA Student Dialogue 2025.', 'success' => false]);
        exit;
    }
            ...
            <body style="margin: 0; padding: 0; font-family: Arial, sans-serif; background-color: #f4f4f4;">
                <div style="max-width: 600px; margin: auto; background-color: #ffffff; padding: 20px;">
                    <div style="text-align: center; padding-bottom: 20px;">
                        <img src="https://yuva.ipnfoundation.org/assets/images/Logo.png" alt="YUVA Logo" style="max-width: 200px;">
                    </div>
                    <div style="padding: 20px; color: #333333;">
                        ...
                        <div style="text-align:center; margin-top:30px;">
                            <a href="https://yuva.ipnfoundation.org/" style="display:inline-block; padding:10px 20px; background-color:#2c5282; color:#ffffff; text-decoration:none; border-radius:5px;">View Event Details</a>
                        </div>
